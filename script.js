...
document.addEventListener("keydown", e => {
  ...
  if (flipbook?.classList.contains("open")) {
...
function renderCatalogPage(item, mode = "book") {
  const id = String(item.id).padStart(2, "0");

  return `
    <figure class="${mode}-image-area">
      <img src="${item.assetBase}.png" alt="${escapeHtml(item.title)}">
    </figure>

    <div class="${mode}-text">
      <div class="${mode}-number">FAKE_${id}</div>

      <h2>${escapeHtml(item.title)}</h2>

      <dl>
        <dt>Year</dt><dd>${escapeHtml(item.year || "Unknown")}</dd>
        <dt>Mode</dt><dd>${escapeHtml(item.realityMode || "")}</dd>
        <dt>Status</dt><dd>${escapeHtml(item.presentStatus || "")}</dd>
        <dt>Value</dt><dd>${escapeHtml(item.valueType || "")}</dd>
      </dl>
...
function buildPrintCatalog() {
  let catalog = document.getElementById("printCatalog");

  if (catalog) catalog.remove();

  catalog = document.createElement("section");
  catalog.id = "printCatalog";
  catalog.className = "print-catalog";

  catalog.innerHTML = `
    <section class="print-cover print-front-cover">
      <img src="asset/cover_front.png" alt="Front Cover">
    </section>

    <section class="print-intro-page">
      <div class="print-intro">
        ${escapeHtml(document.getElementById("introPanel")?.textContent || "")}
      </div>
    </section>
    ...
    <section class="print-list">
      ${ARCHIVE.map(item => `
        <article class="print-item">
          ${renderCatalogPage(item, "print")}
        </article>
      `).join("")}
    </section>

    <section class="print-cover print-back-cover">
      <img src="asset/cover_back.png" alt="Back Cover">
    </section>
  `;
...
function createBookSpreads() {
  const introText = escapeHtml(document.getElementById("introPanel")?.textContent || "");

  const frontCover = `
    <div class="book-cover-content">
      <h1>Fakekammer</h1>
      <p>a generated wunderkammer of doubtful objects</p>
    </div>
  `;
  ...
  const archivePages = ARCHIVE.map(item => renderCatalogPage(item, "book"));
  ...
  spreads.push({
    type: "normal",
    left: introPage,
    right: archivePages[0] || ""
  });

  for (let i = 1; i < archivePages.length; i += 2) {
    spreads.push({
      type: "normal",
      left: archivePages[i] || "",
      right: archivePages[i + 1] || ""
    });
  }
...
function renderSpread() {
  ...
  spreadEl.classList.remove("cover-mode", "back-cover-mode", "spread-mode");

  if (spread.type === "front-cover") {
    spreadEl.classList.add("cover-mode");

    leftPage.innerHTML = spread.left || "";
    leftPage.classList.add("book-cover", "cover-single");
    rightPage.classList.add("page-hidden");
    return;
  }

  if (spread.type === "back-cover") {
    spreadEl.classList.add("back-cover-mode");

    rightPage.innerHTML = spread.right || "";
    rightPage.classList.add("book-cover", "cover-single");
    leftPage.classList.add("page-hidden");
    return;
  }

  spreadEl.classList.add("spread-mode");
...
function nextSpread() {
  if (currentSpread < bookSpreads.length - 1) {
    animateTurn("next", () => {
      currentSpread++;
      renderSpread();
    });
...
flipbookBtn.addEventListener("click", e => {
  e.stopPropagation();
  openFlipbook();
});

closeFlipbookBtn.addEventListener("click", e => {
  e.stopPropagation();
  closeFlipbook();
});
